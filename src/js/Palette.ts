...
export default class Palette {
  ...
  constructor(public window: number, public config) {
    ...
    this.colour_depth = this.config.colour_depth;
    this.window = window;
  ...
  update(all_data): void {
    ...
    dom.remove_all_class("#palette_spritecolors div","palette_color_item_selected");
  ...
  draw_palette(): void {
    ...
    */
    ...
    for (let i = 0; i < this.colour_depth; i++) {
            let picker_div =
...
//   }
  ...
  set_colors(colors, colour_depth): void {
    this.colors = colors;
